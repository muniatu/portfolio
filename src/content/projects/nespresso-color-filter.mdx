...
Around 90% of visitors on the Nespresso website are returning customers buying their usual capsules. New flavors get tried mostly when free samples arrive with an order, not when someone scans the product list. That flips the brief: the job isn't discovery, it's getting the 90% to their basket faster, without closing the door on the 10% who do want to browse.

If that's right, here's what should move:

<KeyPoints>
  <KeyPoint title="Faster recognition.">People scan, they don't read. Time-to-basket drops, drop-off with it.</KeyPoint>
  <KeyPoint title="Smoother repeats.">Easier purchases come back sooner. Reorder frequency without spending on acquisition.</KeyPoint>
  <KeyPoint title="Fewer mis-taps.">Color cuts the added-then-removed pattern. Less back-and-forth before checkout.</KeyPoint>
  <KeyPoint title="Discoverability deferred, not lost.">Descriptions move one tap deeper. The 10% lose nothing, the 90% stop paying for it.</KeyPoint>
</KeyPoints>

So the product card kept what the recurrent task depends on: name, intensity, color. Descriptions and tasting notes moved one tap away.

Then the color filter, sitting at the top of the grid alongside the existing ones. Tap purple, you see purples. Tap purple and brown, you get both. The interaction is unremarkable on purpose. The point isn't the filter as a feature, it's that the page now speaks the language we kept hearing in interviews.

## What we heard

We tested it unmoderated with six users. One task: _"Find the coffee you usually buy."_
